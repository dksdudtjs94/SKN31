...
- 날짜관련 함수
 ...
 timestampdiff(계산할 대상단위, 시작일시, 끝일시) - 계산대상 단위에 맞춰 계산한 기간을 반환. 끝일시 - 시작일시
...
select subdate(curdate(), interval 10 month), -- 이전 날짜
       curdate(),
       adddate(curdate(), interval 10 month)  -- 이후 날짜
;
select now(), adddate(now(), interval 3 week) "3주후",
	   adddate(now(), interval 10 hour) "10시간 후",
       adddate(now(), interval -10 hour) "10시간 전",
       subdate(now(), interval 10 hour) "sub 10시간 전";
